<?php
// ALTER TABLE  `ecm_store` ADD  `store_recom_pic` VARCHAR( 255 ) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL AFTER  `store_logo` ;
/* 店铺控制器 */
class StoreApp extends BackendApp
{
   ...
   function ajax_col()
   {
    ...
    function save_recom_pic(){
        ...
        $store_id = (isset($_POST['sid']) && $_POST['sid'] + 0 > 0) ? $_POST['sid'] + 0 : 0;
        if( isset($_FILES) && $store_id ){
            if($pic = $this->_upload_pic(uniqid())){
                $db = &db();
                $old_pic = $db->getOne('SELECT `store_recom_pic` FROM `'.DB_PREFIX.'store` WHERE `store_id`=\''.$store_id.'\' ');
                if( $old_pic ){
                    @unlink(ROOT_PATH.'/'.$old_pic);
                }
                $db->query('UPDATE `'.DB_PREFIX.'store` SET `store_recom_pic`=\''.$pic.'\' WHERE `store_id`=\''.$store_id.'\' ');
    ...
    function _upload_pic($recompic_id)
    {
        ...
        /* 上传 */
        if ($file_path = $uploader->save('data/files/mall/storerecompic', $recompic_id))   //保存到指定目录，并以指定文件名$arads_id存储
        {
            return $file_path;
        }
        else
