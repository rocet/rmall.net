...
class FrontendApp extends ECBaseApp
{
    ...
    function display($tpl)
    {
        ...
        $cart_number = $cart->get_nums(SESS_ID, $this->visitor->get('user_id'));
        $this->assign('cart_goods_nums', $cart_number);
        setcookie('cart_number',$cart_number, gmtime() + 3600 * 32);
        
        $this->assign('rocet_adv_topad', current( m('arads')->find(array( 
            'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'topad'")) ),
            'limit'   => '0, 1',
            'order'   => 'arads_id desc', 
        ))) );


        $curapp = isset($_GET['app']) ? $_GET['app'] : '';
        $rocet_adv_sitetopads_showns = array('','goods','brand','category','cateindex','search','store');
        $rocet_adv_sitetopads_starts = array_search($curapp, $rocet_adv_sitetopads_showns);
        $rocet_adv_sitetopads_starts = ($rocet_adv_sitetopads_starts ? $rocet_adv_sitetopads_starts : 0) * 12;
        $this->assign('rocet_adv_sitetopads', m('arads')->find(array( 
            'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'sitetpads'")) ),
            'limit'   => $rocet_adv_sitetopads_starts.', 12',
            'order'   => 'arads_id desc', 
        )) );
        ...
        $this->assign('rocet_flinks', m('arflinks')->find(array( 
            'limit'   => '0, 30',
            'order'   => 'arflinks_id desc', 
        )) );
        

        $topcates =  $this->_get_gcatgory_navs();
        $goods_mod = m('goods');
        $indexnavbrands = array();
        foreach (array_keys($topcates) as $key => $topcate) {
            $indexnavbrands[$topcate] = $goods_mod->getCol("SELECT DISTINCT `brand` FROM ".DB_PREFIX."goods WHERE `cate_id_1`=".$topcate." LIMIT 0, 4");
            if($adid = key(m('aradps')->find(array('conditions' => '`identify`='."'cate".($key+1)."ads'")) )){
                $topcates[$topcate]['rocet_indexcateads'] = m('arads')->find(array( 
                    'conditions'  => '`aradps_id`=' . $adid,
                    'limit'   => '0, 4',
                    'order'   => 'arads_id desc', 
                ));
            }
        }
        foreach ($indexnavbrands as $key => $value) {
            foreach ($value as $brand) {
                $topcates[$key]['rocet_navbrands'][] = current( m('brand')->find(array('conditions'=> 'brand_name=\''.$brand.'\'')) );
...
//$this->_D( $topcates );
        ...
        $this->assign('rocet_gcatgorynavs', $topcates );


        $this->assign('rocet_foothelpers', $this->_get_footer_helpers() );

        $this->assign('rocet_settings', $GLOBALS['ECMALL_CONFIG'] );
    ...
    function login()
    {
        ...
        if (!IS_POST)
        {
            ...
            $this->assign('rocet_adv_loginpagead', current( m('arads')->find(array( 
                'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'loginpagead'")) ),
                'limit'   => '0, 1',
                'order'   => 'arads_id desc', 
            ))) );
    ...
    function _get_gcatgory_navs(){
        $gcatgorynavs = array();
        $catlvl0 = m('gcategory')->find(array( 
            'conditions'  => '`parent_id`=0  AND `store_id` = 0 AND `if_show`=1 AND `cate_id` < 1000',
            'limit'   => '0, 6',
    ...
    function _get_footer_helpers(){
        $foothelpers = array();
        $foothelperschannel = m('acategory')->find(array( 
            'conditions'  => '`parent_id`='.key(m('acategory')->find(array('conditions' => '`cate_name`='."'底部帮助'")) ),
            'limit'   => '0, 30',
            'order'   => 'sort_order asc', 
        ));
        foreach ($foothelperschannel as $value) {
            $foothelpers[] = array(
                'channel'   => $value['cate_name'],
                'articles'  => m('article')->find(array( 
...
class ShoppingbaseApp extends MallbaseApp
{
    ...
    {
        /* 只有登录的用户才可访问 */
        if (!$this->visitor->has_login && !in_array(ACT, array('get_newly_sales', 'login', 'register', 'check_user')))
        {
            if (!IS_AJAX)
            {
                header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
