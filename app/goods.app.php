...
class GoodsApp extends StorebaseApp
{
    ...
    function index()
    {
        ...
        $this->assign('rocet_goods_sibling_cat', m('gcategory')->find(array(
            'fields' => 'cate_id, cate_name',
            'conditions' => " parent_id = {$data[goods][cate_id_2]}",
            'limit' => '0, 30',
            'order' => 'sort_order asc',
        )));


        $this->assign('rocet_goods_hot_recommend', m('goods')->find(array(
            'join' => 'belongs_to_store, has_goodsstatistics',
            'fields' => 'g.default_image, g.price, g.goods_name, s.store_id, s.store_name, g.cate_name, g.brand, g.closed, g.if_show, views',
            'conditions' => " g.cate_id = {$data[goods][cate_id]}",
            'limit' => '0, 3',
            'order' => 'RAND()',
        )));

        $this->assign('rocet_goods_buyer_recommend', m('goods')->find(array(
            'join' => 'belongs_to_store, has_goodsstatistics',
            'fields' => 'g.default_image, g.price, g.goods_name, s.store_id, s.store_name, g.cate_name, g.brand, g.closed, g.if_show, views',
            'conditions' => " g.cate_id = {$data[goods][cate_id]}",
            'limit' => '0, 6',
        ...
        $this->assign('rocet_goods_visitor_recommend', m('goods')->find(array(
            'join' => 'belongs_to_store, has_goodsstatistics',
            'fields' => 'g.default_image, g.price, g.goods_name, s.store_id, s.store_name, g.cate_name, g.brand, g.closed, g.if_show, views',
            'conditions' => " g.cate_id = {$data[goods][cate_id]}",
            'limit' => '0, 6',
        ...
        $this->assign('rocet_adv_goods_left', m('arads')->find(array( 
            'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'goodsleft'")) ),
            'limit'   => '0, 3',
            'order'   => 'arads_id desc', 
        )));

        $specinfos = array();
        foreach ($data['goods']['_specs'] as $key => $value) {
            $specinfos['spec_1'][$value['spec_1']] = $value['spec_1'] ;
            $specinfos['spec_2'][$value['spec_2']] = $value['spec_2'] ;
        }
        $specinfos['spec_1'] = array_values($specinfos['spec_1']);
        $specinfos['spec_2'] = array_values($specinfos['spec_2']);

        $this->assign('specinfos',   $specinfos );


// INSERT INTO `ecmall2`.`ecm_mall_attr` 
// (`mattr_id`, `goods_id`, `attr_id_1`, `attr_id_2`, `color_rgb`, `price`, `stock`, `sku`, `val`) 
// VALUES 
// (NULL, '20', '1', '6', '', '0.00', '33', 'dwfew1122112', 'dwdew'), 
// (NULL, '20', '2', '13', '', '0.00', '31', 'sasq12232131', 'dwqdew');

        $attrinfos = m('mallattr')->find(array(
            'conditions' => '`goods_id`='.$id,
        ));
        foreach ($attrinfos as $key => &$value) {
            $value['attr_id_1'] = m('attr')->find(array('conditions'=>'`attr_id`='.$value['attr_id_1']));
            $value['attr_id_2'] = m('attr')->find(array('conditions'=>'`attr_id`='.$value['attr_id_2']));
        }
        $goodattrs = array();
        foreach ($attrinfos as $key => $value) {
            foreach ($value['attr_id_1'] as $key1 => $value1) {
                foreach ($value['attr_id_2'] as $key2 => $value2) {
                    $goodattrs[$value1['attr_name']][] = $value2['attr_name'];
                    $goodattrs[$value1['attr_name']] = array_unique($goodattrs[$value1['attr_name']]);
                }
            }
        }
