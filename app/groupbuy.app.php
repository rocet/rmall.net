...
class GroupbuyApp extends StorebaseApp
{
    ...
    function index()
    {
        ...
        if (!$id)
        {
            $group_datas = $this->_groupbuy_mod->find(array(
            ...
            $cats = array();
            $cats_count = array();
            foreach ($group_datas as & $value) {
                $tmp_group_price = current( unserialize($value['spec_price']) );
                $value['group_price'] =  $tmp_group_price['price'];
                $value['group_discount'] = number_format( $tmp_group_price['price'] / $value['price'] * 10, 1 );
                $value['group_joineds'] = $this->_groupbuy_mod->get_join_quantity($value['group_id']);
                $cate_3_info = current( m('gcategory')->find(array('fields'=>'cate_id,cate_name', 'conditions'=>'`cate_id`='.$value['cate_id'])) );
                $cate_1_info = current( m('gcategory')->find(array('fields'=>'cate_id,cate_name', 'conditions'=>'`cate_id`='.$value['cate_id_1'])) );
                $cats_count['cat_1_count_'.$cate_1_info['cate_id']] = isset($cats_count['cat_1_count_'.$cate_1_info['cate_id']]) ? $cats_count['cat_1_count_'.$cate_1_info['cate_id']] + 1 : 1;
                $cats_count['cat_3_count_'.$value['cate_id']] = isset($cats_count['cat_3_count_'.$value['cate_id']]) ? $cats_count['cat_3_count_'.$value['cate_id']] + 1 : 1;
                $cats['cat_1_count_'.$cate_1_info['cate_id']]['cate_name'] = $cate_1_info['cate_name'];
                $cats['cat_1_count_'.$cate_1_info['cate_id']]['cate_id'] = $cate_1_info['cate_id'];
                $cats['cat_1_count_'.$cate_1_info['cate_id']]['subcat']['cat_3_count_'.$value['cate_id']]['cate_name'] = $cate_3_info['cate_name'];
                $cats['cat_1_count_'.$cate_1_info['cate_id']]['subcat']['cat_3_count_'.$value['cate_id']]['cate_id'] = $cate_3_info['cate_id'];
                $cats['cat_1_count_'.$cate_1_info['cate_id']]['subcat']['cat_3_count_'.$value['cate_id']]['cate_count'] = isset($cats['cat_1_count_'.$cate_1_info['cate_id']]['subcat']['cat_3_count_'.$value['cate_id']]['cate_count']) ? $cats['cat_1_count_'.$cate_1_info['cate_id']]['subcat']['cat_3_count_'.$value['cate_id']]['cate_count'] + 1 : 1;
            }
            $this->assign('rocet_cats_groupcats',  $cats );
            $this->assign('rocet_cats_groupcats_count',  $cats_count );
            // $this->_D($cats);
            $this->assign('rocet_adv_groupactive',      m('arads')->find(array( 
                'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'indexbannerfocus'")) ),
                'limit'   => '0, 3',
                'order'   => 'arads_id desc', 
            )) );
            $this->assign('rocet_adv_groupadtop',  current( m('arads')->find(array( 
                'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'indexbannerfocus'")) ),
                'limit'   => '0, 1',
                'order'   => 'arads_id desc', 
            ))) );
            $this->assign('groupbuys',      $group_datas);
        ...
        if (!IS_POST)
        {
            ...
            $this->display('groupbuy.index-bak.html');
        }
        else
        {
                if (isset($_POST['join']))
