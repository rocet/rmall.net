...
define('THUMB_WIDTH', 565);
define('THUMB_HEIGHT', 374);
...
class My_goodsApp extends StoreadminbaseApp
{
    ...
    function My_goodsApp()
    {
		...
		$this->_seckill_configs = file_exists(ROOT_PATH."/data/seckill.inc.php") ? include(ROOT_PATH."/data/seckill.inc.php") : '';
    }
    ...
    function add()
    {
        ...
        if (!IS_POST)
        {
             ...
             /* 取得商品属性 */
             $this->assign('mallattroptions', m('attr')->get_options());
             $this->assign('mallattrs', m('attr')->get_tree());
             //$this->assign('goods_attrs' m('mallattr')->find(array('conditions'=>'`goods_id`='.)));
    ...
    function edit()
    {
        ...
        if (!IS_POST)
        {
            ...
            $this->assign('mallattroptions', m('attr')->get_options());
            $this->assign('mallattrs', m('attr')->get_tree());
            $goods_attrs = m('mallattr')->find(array('join'=>'belongs_to_attr', 'conditions'=>'`goods_id`='.$id));

            $assign_goods_attrs = '';
            foreach ($goods_attrs as $value) {
                $assign_goods_attrs .= ',{"pid":"'.$value['attr_id_1'].'","id":"'.$value['attr_id_2'].'","name":"'.$value['attr_name'].'"}';
            }
            $this->assign('goods_attrs',  $assign_goods_attrs );
   ...
   function ajax_col()
   {
    ...
    function _get_post_data($id = 0)
    {
		...
		if(isset($_POST['seckill'])){
        	$goods['if_seckill']= $_POST['seckill'];
		}

        if(isset($_POST['market_price'])){
            $goods['market_price']= $_POST['market_price'];
        }

        if(isset($_POST['active_price'])){
            $goods['active_price']= $_POST['active_price'];
        }

        if(isset($_POST['discount'])){
            $goods['discount']= $_POST['discount'];
        }

        if(isset($_POST['active_start_time'])){
            $goods['active_start_time']= strtotime( $_POST['active_start_time']);
        }

        if(isset($_POST['active_end_time'])){
            $goods['active_end_time']= strtotime($_POST['active_end_time']);
        }
    ...
    function _check_post_data($data, $id = 0)
    {
		...
		if ((is_array($this->_seckill_configs)) && $data['goods']['if_seckill'])
        {
	        if($this->_check_seckill_upper_limit($id))
	        {
	        	$this->_error('seckill_upper_limit');
	        	return false;
	        }
        }
        return true;
	...
	function check_seckill_upper_limit()
    {
    	$goods_id = isset($_GET['id']) ? intval($_GET['id']) : '';
    	if(empty($goods_id)){
    		return;
    	}
		echo $this->_check_seckill_upper_limit($goods_id);
    }
	function _check_seckill_upper_limit($goods_id)
    {	
    	if (is_array($this->_seckill_configs))
    	{
	    	$upper_limit = $this->_seckill_configs['goods_qty'];
	    	$current_qty = count($this->_goods_mod->get_today_record($goods_id));
	    	
	    	if($upper_limit == 0){
	    		return false;                        //0不限制
	    	}
	    	
	    	if($current_qty>=$upper_limit)
	    		return 'has_upper_limit';
    	}
        ...
        $nowTime = getdate(gmdate(gmtime()));
        $nowTimeSec = $nowTime['hours']*3600+$nowTime['minutes']*60+$nowTime['seconds'];
        
        if($nowTimeSec >= $sTimeSec){
        	return 'seckill_begin';
        }
    	return false;
    }
    ...
    function _save_post_data($data, $id = 0)
    {
        ...
        if ($id > 0)
        {
            ...
            m('mallattr')->drop(array('conditions' => '`goods_id`='.$goods_id));
            $goods_attrs  = json_decode('['.str_replace('\\', '', trim($_POST['goods_attrs'], ', ') ).']', true);
            foreach ($goods_attrs as $value) {
                m('mallattr')->add(array(
                    'attr_id_1'=>$value['pid'],
                    'attr_id_2'=>$value['id'],
                    'goods_id'=>$goods_id
                ));
            ...
            // foreach ($goods_attrs as $value) {
            //     $nochange = 0;
            //     foreach ($db_goods_attrs as $val) {
            //         if( $val['attr_id_1'] == $value['pid'] && $val['attr_id_2'] == $value['id'] && $val['goods_id'] == $goods_id){
            //             $nochange = 1;
            //         }
            //     }
            //     if(!$nochange){
            ...
            //             'goods_id'=>$goods_id
            //         ) ) );
            //         // m('mallattr')->edit($goods_id, array(
            //         //     'attr_id_1'=>$value['pid'],
            //         //     'attr_id_2'=>$value['id'],
            //         //     'goods_id'=>$goods_id
            //         // ));
