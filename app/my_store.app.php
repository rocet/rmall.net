...
class My_storeApp extends StoreadminbaseApp
{
    ...
    function index()
    {
        ...
        else
        {
            ...
            else //删除冗余图标
            {
                ...
                if($store['store_banner2'] != '' && $data['store_banner2'] != '')
                {
                    $store_banner2_old = pathinfo($store['store_banner2']);
                    $store_banner2_new = pathinfo($data['store_banner2']);
                    if($store_banner2_old['extension'] != $store_banner2_new['extension'])
                    {
                        unlink($store['store_banner2']);
                    }
                }
            }
    ...
    function _upload_files()
    {
        ...
        if ($file['error'] == UPLOAD_ERR_OK && $file !='')
        {
            ...
            $uploader->root_dir(ROOT_PATH);
            $data['store_banner2'] = $uploader->save('data/files/store_' . $this->_store_id . '/other', 'store_banner2');
        }
