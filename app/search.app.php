...
class SearchApp extends MallbaseApp
{
    ...
    {
        // 查询参数
        isset($_GET['keyword']) && is_array($_GET['keyword']) && $_GET['keyword'] = implode(' ', $_GET['keyword']);
        isset($_POST['keyword']) && is_array($_POST['keyword']) && $_POST['keyword'] = implode(' ', $_POST['keyword']);
        isset($_REQUE['keyword']) && is_array($_REQUE['keyword']) && $_REQUE['keyword'] = implode(' ', $_REQUE['keyword']);

        $param = $this->_get_query_param();
        if (empty($param))
        {
            header('Location: index.php?app=cateindex&cate_id=1');
            exit;
        }
        ...
        $goods_list = $goods_mod->get_list(array(
            ...
            'order'      => isset($_GET['order']) && isset($orders[urldecode( $_GET['order']) ]) ? urldecode($_GET['order']) : '',
            'limit'      => $page['limit'],
        ));
        ...
        $gm = & bm('gcategory');
        $parents = $gm->get_ancestor($cate_id, true);
        // 分类ID lv1
        $this->assign('cate_id_1', $parents[0] ? $parents[0]['cate_id'] : 0 );
        // 分类ID lv2
        $this->assign('cate_id_2', $parents[1] ? $parents[1]['cate_id'] : 0 );
        // 分类ID lv3
        $this->assign('cate_id_3', $parents[2] ? $parents[2]['cate_id'] : 0 );
        ...
        //  本月推荐
        $this->assign('rocet_search_tuijian', m('goods')->find(array(
            ...
            'conditions' => " g.cate_id = $cate_id AND recommended_goods.recom_id = '".key(m('recommend')->find(array(
        ...
        //  本分类上部商品推荐
        $this->assign('rocet_search_adtuijian', m('goods')->find(array(
            'join' => 'be_recommend, belongs_to_store, has_goodsstatistics',
        ...
        // 搜索页顶部广告
        $this->assign('rocet_adv_search_topad', current( m('arads')->find(array( 
            'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'searchtopad'")) ),
            'limit'   => '0, 1',
            'order'   => 'arads_id desc', 
        ))) );
        ...
        if( method_exists($this, '_extendindex') ){
            $this->_extendindex(get_defined_vars());
        }
    ...
    function _get_goods_curlocal($cate_id)
    {
        ...
        {
            $curlocal[] = array('id' => $category['cate_id'], 'text' => $category['cate_name'], 'url' => "javascript:replaceParam('cate_id', '" . $category['cate_id'] . "')");
        }
    ...
    function _get_store_curlocal($cate_id)
    {
        ...
        foreach ($parents as $category)
        {
            $curlocal[] = array('id' => $category['cate_id'], 'text' => $category['cate_name'], 'url' => url('app=search&act=store&cate_id=' . $category['cate_id']));
        }
        unset($curlocal[count($curlocal) - 1]['url']);
        return $curlocal;
    }
