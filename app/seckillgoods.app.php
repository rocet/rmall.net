...
class SeckillgoodsApp extends MallbaseApp
{
    ...
    function index(){
        ...
        $this->assign('rocet_adv_seckillgoodsbanner',current( m('arads')->find(array( 
            'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'seckillgoodsbanner'")) ),
            'limit'   => '0, 1',
            'order'   => 'arads_id desc', 
        ))) );

        $this->assign('rocet_adv_seckillgoodsrightad', current( m('arads')->find(array( 
            'conditions'  => '`aradps_id`=' . key(m('aradps')->find(array('conditions' => '`identify`='."'seckillgoodsrightad'")) ),
            'limit'   => '0, 1',
            'order'   => 'arads_id desc', 
        ))) );
        ...
        $this->assign('rocet_seckillgoods_hot', m('goods')->find(array(
        ...
        // $this->assign('rocet_seckillgoods_goods', m('seckill')->find(array(
        //     'join' => 'belong_goods, belongs_to_store, has_goodsstatistics',
        //     'fields' => 'g.default_image, g.price, g.goods_name, s.store_id, s.store_name, g.cate_name, g.brand, recommended_goods.recommend_pic, recommended_goods.sort_order, g.closed, g.if_show, views',
        //     'conditions' => "recommended_goods.recom_id = '".key(m('recommend')->find(array(
        ...
        // )) );
        $this->seckill_list();
    	$this->display('goods.seckill.html');
    }
        ...
        foreach ($seckill_info as $sk => $sv){ 
            ...
            if(is_numeric($leave_time) && $leave_time > 0 && $seckill_info[$sk]['sec_state'] != SECKILL_END ){
                if($seckill_info[$sk]['start_time'] >= (mktime(0,0,0,$today['mon'],$today['mday'],$today['year'])) && $seckill_info[$sk]['start_time'] <= $end_time && $start_leave_time > 0 && count($current_seckill_info) <= 3){
                    ...
                    $current_seckill_info[$sk]['end_time'] = $end_time;
                    //更新状态为开始
...
// exit(  var_dump( $current_seckill_info ) );

// $a1 = (int)($start_leave_time/3600).':'.(int)($start_leave_time%3600/60).':'.$start_leave_time%3600%60;
// $a2 = is_numeric($leave_time) == TRUE ? (int)($leave_time/3600).':'.(int)($leave_time%3600/60).':'.$leave_time%3600%60 : $leave_time;
//         exit(  var_dump($a1, $a2, $start_leave_time, $leave_time ) );
    }
