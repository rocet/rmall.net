...
class GoodsModel extends BaseModel
{
    ...
    function get_list($params = array(), $scate_ids = array(), $desc = false, $no_picture = true)
    {
        ...
        $gattrs = ( isset($_GET['gattrs']) && is_array($_GET['gattrs']) ) ? $_GET['gattrs'] : 0;
        ...
        $tables = "{$this->table} g " .
                ...
                ($gattrs ? "LEFT JOIN {$attr_mod->table} ga ON g.goods_id = ga.goods_id " : '') .
        ...
        $conditions .= " AND s.store_id IS NOT NULL "; //AND gs.spec_id IS NOT NULL


        if( $gattrs ){
            foreach ($_GET['gattrs'] as $k => $v) {
                $conditions .= " and exists (select * from ecm_mall_attr ga".$k.
                " where (ga".$k.".attr_id_2 = '".$v."') and exists (select * from ecm_mall_attr gab".$k.
                " where gab".$k.".goods_id = g.goods_id and gab".$k.".attr_id_2 = ga".$k.".attr_id_2))";
            }
        }
    ...
    function get_info($id)
    {
        ...
        if ($goods)
        {
            ...
            /* 商品属性 */                       //
            $attr_mod =& m('mallattr');
            $attrs = $attr_mod->find(array(
                'conditions' => "goods_id = '$id'",
                'order' => 'mattr_id',
            ));
            $goods['_attrs'] = array_values($attrs);
...
class GoodsBModel extends GoodsModel
{
	...
	function get_today_record($goods_id = 0)
    {
    	$sql = "SELECT * FROM {$this->table} WHERE UNIX_TIMESTAMP(NOW())-last_update<=86400 and if_show=1 and if_seckill=1 and goods_id<>$goods_id order by last_update";
